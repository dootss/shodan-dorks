...
`product:"Hikvision IP Camera"` - 2,846,627 results  
...
`title:"IPCam Client"` - 64,097 results  
...
`server: VVTK-HTTP-Server` - 16,734 results  
...
`port:502` - 516,563 results  


[S7](https://www.shodan.io/search?query=port%3A102)  
`port:102` - 516,444 results  
...
`"in-tank inventory" port:10001` - 6,282 results  
...
`"DICOM Server Response" port:104` - 1,821 results  
...
`port:20000 source address` - 1,051 results  
...
`smart install client active` - 7,649 results  
...
`"Android Debug Bridge" "Device" port:5555` - 4,756 results  
