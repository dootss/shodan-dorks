...
`product:"Yawcam webcam viewer httpd"` - 632 results  
...
`"in-tank inventory" port:10001` - 5,925 results  
...
`"authentication disabled" port:5900,5901` - 5,432 results  
...
`port:9600 response code` - 1,776 results  


[PCWorx](https://www.shodan.io/search?query=port%3A1962%20PLC)  
`port:1962 PLC` - 948 results  
...
`"root@" port:23 -login -password -name -Session` - 1,818 results  
...
`kibana content-length:217` - 628 results  
...
`"Set-Cookie: mongo-express=" "200 OK"` - 477 results  
...
`title:"Weave Scope" http.favicon.hash:567176827` - 20 results  
...
*last updated at: 2024-03-03 05:44:30*
