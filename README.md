...
`product:"Hikvision IP Camera"` - 2,848,513 results  
...
`title:"IPCam Client"` - 65,005 results  
...
`webcam has_screenshot:true` - 224 results  
...
`"in-tank inventory" port:10001` - 6,330 results  
...
`port:2404 asdu address` - 3,629 results  
...
`port:9600 response code` - 1,960 results  
...
`mongodb port:27017` - 109,944 results  
...
`"Set-Cookie: mongo-express=" "200 OK"` - 435 results  
...
`title:"syncthru web service"` - 1,033 results  
...
`http.title:"Hacked by"` - 545 results  
...
`http.title:"dashboard"` - 351,018 results  
...
`"Minecraft Server" "protocol 340" port:25565` - 11,274 results  
