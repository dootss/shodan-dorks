...
`title:"IPCam Client"` - 63,658 results  
...
`server: VVTK-HTTP-Server` - 16,627 results  
...
`"in-tank inventory" port:10001` - 6,266 results  
...
`"authentication disabled" "RFB 003.008"` - 5,451 results  
...
`port:2404 asdu address` - 3,596 results  
...
`port:20000 source address` - 1,045 results  
...
`"root@" port:23 -login -password -name -Session` - 534 results  
...
`"Set-Cookie: mongo-express=" "200 OK"` - 432 results  
...
`http.title:"Index of /"` - 381,107 results  
...
`"Authentication: disabled" port:445 product:"Samba"` - 268,907 results  


[Filezilla FTP](https://www.shodan.io/search?query=filezilla%20port%3A%2221%22)  
`filezilla port:"21"` - 257,771 results  
...
`hacked-router-help-sos` - 822 results  
