...
`WWW-Authenticate: "Merit LILIN Ent. Co., Ltd"` - 498 results
...
`"Server: EIG Embedded Web Server" "200 Document follows"` - 205,110 results
<br>
...
`"authentication disabled" "RFB 003.008"` - 52,061 results
<br>
...
`"HID VertX" port:4070` - 104 results
...
`[1m[35mWelcome on console` - 18 results
<br>
...
`"root@" port:23 -login -password -name -Session` - 558 results
...
`Lantronix password port:30718 -secured` - 504 results
...
`"Docker-Distribution-Api-Version: registry" "200 OK" -gitlab` - 241 results

[Accessible Kibana Dashboards](https://www.shodan.io/search?query=kibana%20content-length%3A217)  
`kibana content-length:217` - 220 results
...
`title:"OctoPrint" -title:"Login" http.favicon.hash:1307375944` - 6 results
...
`port:4444 system32` - 1,467 results
...
`http.title:"Hacked by"` - 300 results
...
`HACKED-ROUTER-HELP-SOS-HAD-DEFAULT-PASSWORD` - 26 results

[Compromised FTP Servers](https://www.shodan.io/search?query=HACKED%20FTP%20server)  
`HACKED FTP server` - 17 results
...
`http.title:"dashboard"` - 384,377 results
...
*last updated at: 2026-04-30 15:28:05*
