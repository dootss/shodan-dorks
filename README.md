...
`WWW-Authenticate: "Merit LILIN Ent. Co., Ltd."` - 1,153 results  
...
`"in-tank inventory" port:10001` - 5,628 results  
...
`"authentication disabled" port:5900,5901` - 4,954 results  
...
`port:9600 response code` - 1,642 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 7,160 results  


[Cisco Smart Install](https://www.shodan.io/search?query=smart%20install%20client%20active)  
`smart install client active` - 6,818 results  
...
`"root@" port:23 -login -password -name -Session` - 480 results  
