...
`"in-tank inventory" port:10001` - 6,076 results  
...
`"authentication disabled" port:5900,5901` - 5,775 results  
...
`port:2404 asdu address` - 3,210 results  
...
`port:9600 response code` - 1,749 results  


[PCWorx](https://www.shodan.io/search?query=port%3A1962%20PLC)  
`port:1962 PLC` - 950 results  
...
`"root@" port:23 -login -password -name -Session` - 1,881 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 1,283 results  
...
`"Set-Cookie: mongo-express=" "200 OK"` - 512 results  
...
`http.title:"Index of /"` - 385,169 results  
...
*last updated at: 2023-12-31 22:55:54*
