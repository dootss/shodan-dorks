...
`200 ok dvr port:"81"` - 10,077 results  
...
`"authentication disabled" port:5900,5901` - 5,852 results  
...
`port:2404 asdu address` - 3,618 results  
...
`port:20000 source address` - 1,022 results  


[ProConOS](https://www.shodan.io/search?query=port%3A20547%20PLC)  
`port:20547 PLC` - 515 results  
...
`"MongoDB Server Information" port:27017` - 107,605 results  
...
`"root@" port:23 -login -password -name -Session` - 543 results  
...
`http 200 server epson -upnp` - 3,001 results  
...
`"HP-ChaiSOE" port:"80"` - 166 results  
...
`http.title:"Index of /"` - 381,994 results  
...
`hacked-router-help-sos` - 824 results  
