...
`product:"Yawcam webcam viewer httpd"` - 553 results  
...
`"authentication disabled" port:5900,5901` - 5,917 results  
...
`"Siemens, SIMATIC" port:161` - 3,030 results  
...
`port:"9200" all:elastic` - 30,036 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 7,531 results  
...
`http.title:"- Polycom" "Server: lighttpd"` - 3,626 results  
...
`Printer Type: Lexmark` - 129 results  
...
`http.title:"control panel"` - 62,485 results  
