...
`WWW-Authenticate: "Merit LILIN Ent. Co., Ltd."` - 1,281 results  
...
`webcam has_screenshot:true` - 225 results  
...
`"in-tank inventory" port:10001` - 6,062 results  
...
`"authentication disabled" port:5900,5901` - 5,730 results  
...
`"Siemens, SIMATIC" port:161` - 3,036 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 1,298 results  
...
`"Set-Cookie: mongo-express=" "200 OK"` - 515 results  
...
`http.title:"Index of /"` - 384,195 results  
...
`port:4444 system32` - 1,031 results  
...
`http.title:"Hacked by"` - 651 results  
...
`HACKED-ROUTER-HELP-SOS-HAD-DEFAULT-PASSWORD` - 145 results  


[Compromised FTP Servers](https://www.shodan.io/search?query=HACKED%20FTP%20server)  
`HACKED FTP server` - 89 results  
...
*last updated at: 2024-01-01 18:46:30*
