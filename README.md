...
`product:"Yawcam webcam viewer httpd"` - 571 results  
...
`"in-tank inventory" port:10001` - 5,646 results  
...
`"authentication disabled" port:5900,5901` - 4,899 results  
...
`port:20000 source address` - 912 results  


[PCWorx](https://www.shodan.io/search?query=port%3A1962%20PLC)  
`port:1962 PLC` - 830 results  
...
`"root@" port:23 -login -password -name -Session` - 733 results  
...
`title:"syncthru web service"` - 925 results  
...
`"HP-ChaiSOE" port:"80"` - 167 results  
...
*last updated at: 2024-03-26 03:24:32*
