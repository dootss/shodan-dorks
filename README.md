...
`webcam has_screenshot:true` - 230 results  
...
`has_screenshot:true IP Webcam` - 116 results  
...
`"authentication disabled" port:5900,5901` - 5,816 results  
...
`Lantronix password port:30718 -secured` - 641 results  
...
`"root@" port:23 -login -password -name -Session` - 552 results  
...
`http.title:"Index of /"` - 375,175 results  
...
`HACKED-ROUTER-HELP-SOS-HAD-DEFAULT-PASSWORD` - 105 results  
