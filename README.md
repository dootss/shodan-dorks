...
`webcam has_screenshot:true` - 278 results  
...
`"authentication disabled" port:5900,5901` - 5,384 results  
...
`port:20000 source address` - 829 results  
...
`[1m[35mWelcome on console` - 59 results  
...
`"dnsmasq-pi-hole" "Recursion: enabled"` - 2,966 results  
...
`"root@" port:23 -login -password -name -Session` - 1,834 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 1,347 results  
...
`PBX "gateway console" -password port:23` - 164 results  
...
`http.title:"Index of /"` - 380,766 results  
...
`"Authentication: disabled" port:445 product:"Samba"` - 270,654 results  


[Filezilla FTP](https://www.shodan.io/search?query=filezilla%20port%3A%2221%22)  
`filezilla port:"21"` - 267,627 results  
...
`http.title:"Hacked by"` - 832 results  
...
*last updated at: 2024-01-16 03:16:13*
