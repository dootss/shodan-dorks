...
`title:"IPCam Client"` - 62,055 results  
...
`"authentication disabled" "RFB 003.008"` - 5,775 results  
...
`port:2404 asdu address` - 3,556 results  
...
`port:9600 response code` - 1,874 results  
...
`port:20000 source address` - 1,023 results  
...
`mongodb port:27017` - 105,633 results  
...
`"root@" port:23 -login -password -name -Session` - 591 results  
...
`"Authentication: disabled" port:445 product:"Samba"` - 230,116 results  


[Open Lists on Port 80](https://www.shodan.io/search?query=port%3A80%20title%3A%22Index%20of%20/%22)  
`port:80 title:"Index of /"` - 141,893 results  
...
`hacked-router-help-sos` - 777 results  
...
`http.title:"dashboard"` - 199,958 results  
