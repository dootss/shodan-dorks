...
`title:"IPCam Client"` - 58,077 results  
...
`WWW-Authenticate: "Merit LILIN Ent. Co., Ltd."` - 1,147 results  
...
`port:1911,4911 product:Niagara` - 9,200 results  
...
`port:2404 asdu address` - 3,295 results  
...
`"HID VertX" port:4070` - 190 results  
...
`"MongoDB Server Information" port:27017` - 102,030 results  
...
`smart install client active` - 6,825 results  
...
`"Android Debug Bridge" "Device" port:5555` - 4,345 results  
...
`"dnsmasq-pi-hole" "Recursion: enabled"` - 2,747 results  
...
`Server: CANON HTTP Server` - 9,440 results  
...
`http.title:"Index of /"` - 358,851 results  
...
`"Authentication: disabled" port:445 product:"Samba"` - 190,127 results  
...
`hacked-router-help-sos` - 719 results  
...
`http.title:"dashboard"` - 111,215 results  
