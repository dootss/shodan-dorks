...
`WWW-Authenticate: "Merit LILIN Ent. Co., Ltd."` - 1,138 results  
...
`"in-tank inventory" port:10001` - 5,580 results  
...
`"authentication disabled" port:5900,5901` - 4,806 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 6,885 results  


[Cisco Smart Install](https://www.shodan.io/search?query=smart%20install%20client%20active)  
`smart install client active` - 6,795 results  
...
`"root@" port:23 -login -password -name -Session` - 608 results  
...
`port:4444 system32` - 1,075 results  
...
`hacked-router-help-sos` - 646 results  
...
`HACKED-ROUTER-HELP-SOS-HAD-DEFAULT-PASSWORD` - 89 results  
...
*last updated at: 2024-03-27 06:28:37*
