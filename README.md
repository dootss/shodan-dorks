...
`webcam has_screenshot:true` - 120 results
...
`"Siemens, SIMATIC" port:161` - 3,031 results
...
`"DICOM Server Response" port:104` - 2,161 results
...
`"Server: gSOAP/2.8" "Content-Length: 583"` - 23 results
...
`http.title:"- Polycom" "Server: lighttpd"` - 2,337 results
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 1,519 results
...
`"root@" port:23 -login -password -name -Session` - 369 results
...
`"Docker-Distribution-Api-Version: registry" "200 OK" -gitlab` - 222 results

[PBX IP Phone Gateways](https://www.shodan.io/search?query=PBX%20%22gateway%20console%22%20-password%20port%3A23)  
`PBX "gateway console" -password port:23` - 174 results
...
`"attention" "encrypted" port:3389` - 46 results
...
`"Server: Bomgar" "200 OK"` - 386 results
...
*last updated at: 2025-04-05 16:32:51*
