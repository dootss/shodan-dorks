...
`product:"Yawcam webcam viewer httpd"` - 704 results  
...
`"authentication disabled" "RFB 003.008"` - 6,752 results  
...
`"in-tank inventory" port:10001` - 6,102 results  
...
`port:2404 asdu address` - 3,351 results  
...
`port:9600 response code` - 1,739 results  


[PCWorx](https://www.shodan.io/search?query=port%3A1962%20PLC)  
`port:1962 PLC` - 875 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 6,844 results  


[Listed Apache CouchDB](https://www.shodan.io/search?query=product%3A%22CouchDB%22)  
`product:"CouchDB"` - 4,444 results  
...
`"dnsmasq-pi-hole" "Recursion: enabled"` - 2,593 results  
...
`Lantronix password port:30718 -secured` - 694 results  
...
`"Set-Cookie: mongo-express=" "200 OK"` - 595 results  
...
`http.title:"Index of /"` - 391,027 results  
...
`http.title:"Hacked by"` - 853 results  
...
*last updated at: 2024-01-31 16:55:43*
