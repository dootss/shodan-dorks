...
`Netwave IP Camera Content-Length: 2574` - 1,281 results  
...
`WWW-Authenticate: "Merit LILIN Ent. Co., Ltd."` - 1,234 results  
...
`webcam has_screenshot:true` - 229 results  
...
`"in-tank inventory" port:10001` - 6,216 results  
...
`"authentication disabled" port:5900,5901` - 5,805 results  
...
`"authentication disabled" "RFB 003.008"` - 5,348 results  
...
`kibana content-length:217` - 553 results  
...
`"root@" port:23 -login -password -name -Session` - 548 results  
...
`"Set-Cookie: mongo-express=" "200 OK"` - 451 results  
...
`PBX "gateway console" -password port:23` - 210 results  
...
`"HP-ChaiSOE" port:"80"` - 154 results  
...
`http.title:"Index of /"` - 373,188 results  
...
`"Authentication: disabled" port:445 product:"Samba"` - 284,978 results  


[Filezilla FTP](https://www.shodan.io/search?query=filezilla%20port%3A%2221%22)  
`filezilla port:"21"` - 255,059 results  
...
`http.title:"control panel"` - 66,411 results  
