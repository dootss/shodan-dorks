...
`title:"IPCam Client"` - 64,275 results  
...
`server: VVTK-HTTP-Server` - 17,503 results  
...
`webcam has_screenshot:true` - 212 results  
...
`"Siemens, SIMATIC" port:161` - 2,971 results  
...
`port:9600 response code` - 1,774 results  
...
`"dnsmasq-pi-hole" "Recursion: enabled"` - 2,433 results  
...
`"root@" port:23 -login -password -name -Session` - 1,827 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 1,204 results  
...
`Lantronix password port:30718 -secured` - 733 results  
...
`title:"syncthru web service"` - 1,151 results  
...
`http.title:"Index of /"` - 386,007 results  
...
`"Authentication: disabled" port:445 product:"Samba"` - 276,807 results  


[Filezilla FTP](https://www.shodan.io/search?query=filezilla%20port%3A%2221%22)  
`filezilla port:"21"` - 268,927 results  
...
`http.title:"Hacked by"` - 625 results  
...
*last updated at: 2023-12-12 21:43:45*
