...
`webcam has_screenshot:true` - 236 results  
...
`has_screenshot:true IP Webcam` - 122 results  
...
`"authentication disabled" port:5900,5901` - 5,748 results  
...
`"authentication disabled" "RFB 003.008"` - 5,345 results  
...
`"Server: gSOAP/2.8" "Content-Length: 583"` - 43 results  
...
`"dnsmasq-pi-hole" "Recursion: enabled"` - 3,041 results  
...
`Lantronix password port:30718 -secured` - 643 results  
...
`"root@" port:23 -login -password -name -Session` - 547 results  
...
`"Set-Cookie: mongo-express=" "200 OK"` - 449 results  
...
`PBX "gateway console" -password port:23` - 202 results  
...
`http.title:"Index of /"` - 376,346 results  
...
`HACKED-ROUTER-HELP-SOS-HAD-DEFAULT-PASSWORD` - 107 results  
