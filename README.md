...
`webcam has_screenshot:true` - 189 results  
...
`"in-tank inventory" port:10001` - 5,843 results  
...
`port:2404 asdu address` - 3,191 results  
...
`Lantronix password port:30718 -secured` - 626 results  
...
`"root@" port:23 -login -password -name -Session` - 529 results  
...
`HACKED-ROUTER-HELP-SOS-HAD-DEFAULT-PASSWORD` - 111 results  


[Compromised FTP Servers](https://www.shodan.io/search?query=HACKED%20FTP%20server)  
`HACKED FTP server` - 58 results  
