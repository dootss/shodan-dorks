...
`"authentication disabled" port:5900,5901` - 5,940 results  
...
`"authentication disabled" "RFB 003.008"` - 5,588 results  
...
`"DICOM Server Response" port:104` - 1,871 results  
...
`"root@" port:23 -login -password -name -Session` - 613 results  
...
`PBX "gateway console" -password port:23` - 214 results  
...
`port:23 "Password is not set"` - 422 results  
...
`http.title:"Index of /"` - 376,701 results  
