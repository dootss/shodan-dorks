...
`200 ok dvr port:"81"` - 9,784 results  
...
`"authentication disabled" port:5900,5901` - 5,808 results  
...
`"authentication disabled" "RFB 003.008"` - 5,733 results  
...
`port:2404 asdu address` - 3,498 results  
...
`http.title:"- Polycom" "Server: lighttpd"` - 3,680 results  
...
`"root@" port:23 -login -password -name -Session` - 579 results  
...
`"Set-Cookie: mongo-express=" "200 OK"` - 410 results  
...
`"attention" "encrypted" port:3389` - 11 results  
