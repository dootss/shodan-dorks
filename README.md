...
`webcam has_screenshot:true` - 180 results  
...
`has_screenshot:true IP Webcam` - 75 results  
...
`"authentication disabled" port:5900,5901` - 5,944 results  
...
`"in-tank inventory" port:10001` - 5,886 results  
...
`port:2404 asdu address` - 3,246 results  
...
`[1m[35mWelcome on console` - 80 results  
...
`http.title:"Nordex Control" "Windows 2000 5.0 x86" "Jetty/3.1 (JSP 1.1; Servlet 2.2; java 1.6.0_14)"` - 31 results  
...
`"Server: gSOAP/2.8" "Content-Length: 583"` - 28 results  
...
`Lantronix password port:30718 -secured` - 633 results  
...
`"Set-Cookie: mongo-express=" "200 OK"` - 398 results  
...
`PBX "gateway console" -password port:23` - 188 results  
...
`"Docker-Distribution-Api-Version: registry" "200 OK" -gitlab` - 137 results  


[Telnet Configuration](https://www.shodan.io/search?query=%22Polycom%20Command%20Shell%22%20-failed%20port%3A23)  
`"Polycom Command Shell" -failed port:23` - 54 results  
...
`"HP-ChaiSOE" port:"80"` - 149 results  
...
`HACKED-ROUTER-HELP-SOS-HAD-DEFAULT-PASSWORD` - 108 results  
