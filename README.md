...
`product:"Yawcam webcam viewer httpd"` - 512 results  
...
`"authentication disabled" port:5900,5901` - 5,686 results  
...
`port:2404 asdu address` - 3,012 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 6,836 results  


[Cisco Smart Install](https://www.shodan.io/search?query=smart%20install%20client%20active)  
`smart install client active` - 6,812 results  
...
`"root@" port:23 -login -password -name -Session` - 469 results  
...
`http 200 server epson -upnp` - 1,826 results  
...
`hacked-router-help-sos` - 672 results  
