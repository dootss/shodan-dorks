...
`WWW-Authenticate: "Merit LILIN Ent. Co., Ltd."` - 1,177 results  
...
`"in-tank inventory" port:10001` - 5,729 results  
...
`"authentication disabled" port:5900,5901` - 5,116 results  
...
`port:20000 source address` - 889 results  
...
`product:MySQL` - 3,154,764 results  
...
`mongodb port:27017` - 96,602 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 7,917 results  


[Cisco Smart Install](https://www.shodan.io/search?query=smart%20install%20client%20active)  
`smart install client active` - 7,078 results  
...
`http.title:"- Polycom" "Server: lighttpd"` - 3,759 results  
...
`"root@" port:23 -login -password -name -Session` - 1,255 results  
...
`kibana content-length:217` - 595 results  
...
`"Set-Cookie: mongo-express=" "200 OK"` - 421 results  
...
`"Authentication: disabled" port:445 product:"Samba"` - 174,222 results  


[Open Lists on Port 80](https://www.shodan.io/search?query=port%3A80%20title%3A%22Index%20of%20/%22)  
`port:80 title:"Index of /"` - 140,138 results  
...
`port:4444 system32` - 1,332 results  
...
*last updated at: 2024-03-15 23:48:11*
