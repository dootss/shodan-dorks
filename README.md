...
`"in-tank inventory" port:10001` - 6,112 results  
...
`"authentication disabled" port:5900,5901` - 5,561 results  
...
`"Siemens, SIMATIC" port:161` - 3,117 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 5,051 results  


[Listed Apache CouchDB](https://www.shodan.io/search?query=product%3A%22CouchDB%22)  
`product:"CouchDB"` - 4,445 results  
...
`"root@" port:23 -login -password -name -Session` - 1,943 results  
...
`kibana content-length:217` - 642 results  
...
`http.title:"Index of /"` - 389,488 results  
...
`hacked-router-help-sos` - 874 results  
...
*last updated at: 2024-01-29 11:40:04*
