...
`"in-tank inventory" port:10001` - 5,680 results  
...
`"authentication disabled" port:5900,5901` - 5,598 results  
...
`port:9600 response code` - 1,660 results  


[DNP3](https://www.shodan.io/search?query=port%3A20000%20source%20address)  
`port:20000 source address` - 950 results  
...
`"MongoDB Server Information" port:27017` - 97,781 results  
...
`"dnsmasq-pi-hole" "Recursion: enabled"` - 2,629 results  
...
`"root@" port:23 -login -password -name -Session` - 490 results  
...
`filezilla port:"21"` - 238,498 results  
