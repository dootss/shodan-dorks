...
`title:"IPCam Client"` - 63,263 results  
...
`port:9600 response code` - 1,942 results  
...
`"MongoDB Server Information" port:27017` - 109,076 results  


[Default MongoDB Instances](https://www.shodan.io/search?query=mongodb%20port%3A27017)  
`mongodb port:27017` - 109,038 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 16,780 results  
...
`"root@" port:23 -login -password -name -Session` - 604 results  
...
`http.title:"Index of /"` - 374,400 results  
...
`port:4444 system32` - 1,198 results  
