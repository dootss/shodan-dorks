...
`200 ok dvr port:"81"` - 8,342 results  
...
`product:"Yawcam webcam viewer httpd"` - 516 results  
...
`"authentication disabled" port:5900,5901` - 5,750 results  
...
`"in-tank inventory" port:10001` - 5,546 results  
...
`"Siemens, SIMATIC" port:161` - 2,792 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 7,270 results  


[Cisco Smart Install](https://www.shodan.io/search?query=smart%20install%20client%20active)  
`smart install client active` - 6,783 results  
...
`"Android Debug Bridge" "Device" port:5555` - 4,014 results  
...
`"dnsmasq-pi-hole" "Recursion: enabled"` - 2,596 results  
...
`"root@" port:23 -login -password -name -Session` - 472 results  
...
`Server: CANON HTTP Server` - 9,097 results  
