...
`"in-tank inventory" port:10001` - 5,928 results  
...
`"authentication disabled" port:5900,5901` - 5,411 results  
...
`port:20000 source address` - 830 results  
...
`"HID VertX" port:4070` - 194 results  
...
`"root@" port:23 -login -password -name -Session` - 1,819 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 1,358 results  
...
`kibana content-length:217` - 657 results  
...
`"Set-Cookie: mongo-express=" "200 OK"` - 540 results  
...
`ssl:"Xerox Generic Root"` - 388 results  
...
`Printer Type: Lexmark` - 143 results  
...
`http.title:"Index of /"` - 380,739 results  
...
`http.title:"Hacked by"` - 827 results  
...
`HACKED-ROUTER-HELP-SOS-HAD-DEFAULT-PASSWORD` - 128 results  
...
*last updated at: 2024-01-15 13:40:17*
