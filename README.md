...
`server: GeoHttpServer` - 43,831 results  
...
`"authentication disabled" port:5900,5901` - 6,031 results  
...
`"authentication disabled" "RFB 003.008"` - 5,612 results  
...
`"Siemens, SIMATIC" port:161` - 3,186 results  


[Omron FINS](https://www.shodan.io/search?query=port%3A9600%20response%20code)  
`port:9600 response code` - 1,970 results  
...
`port:"9200" all:elastic` - 32,385 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 18,619 results  
...
`http 200 server epson -upnp` - 3,153 results  
...
`ssl:"Xerox Generic Root"` - 438 results  
...
`http.title:"Index of /"` - 376,669 results  
