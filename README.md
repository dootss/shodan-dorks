...
`webcam has_screenshot:true` - 200 results  
...
`"in-tank inventory" port:10001` - 5,599 results  
...
`"authentication disabled" port:5900,5901` - 4,900 results  
...
`"root@" port:23 -login -password -name -Session` - 828 results  
...
`"Set-Cookie: mongo-express=" "200 OK"` - 401 results  
...
`title:"syncthru web service"` - 928 results  
...
`port:23 "Password is not set"` - 401 results  
...
`"HP-ChaiSOE" port:"80"` - 179 results  
...
`HACKED-ROUTER-HELP-SOS-HAD-DEFAULT-PASSWORD` - 96 results  


[Compromised FTP Servers](https://www.shodan.io/search?query=HACKED%20FTP%20server)  
`HACKED FTP server` - 64 results  
...
*last updated at: 2024-03-24 00:24:28*
