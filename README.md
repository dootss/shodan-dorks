...
`product:"Yawcam webcam viewer httpd"` - 521 results  
...
`"in-tank inventory" port:10001` - 5,504 results  
...
`"authentication disabled" port:5900,5901` - 4,784 results  
...
`port:2404 asdu address` - 3,014 results  
...
`port:9600 response code` - 1,610 results  


[DNP3](https://www.shodan.io/search?query=port%3A20000%20source%20address)  
`port:20000 source address` - 926 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 6,589 results  
...
`"Android Debug Bridge" "Device" port:5555` - 4,057 results  
...
`"dnsmasq-pi-hole" "Recursion: enabled"` - 2,605 results  
...
`"root@" port:23 -login -password -name -Session` - 457 results  
...
`"Set-Cookie: mongo-express=" "200 OK"` - 408 results  
