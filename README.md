...
`server: VVTK-HTTP-Server` - 17,106 results  
...
`product:"Yawcam webcam viewer httpd"` - 591 results  
...
`"authentication disabled" port:5900,5901` - 5,765 results  
...
`port:2404 asdu address` - 3,162 results  
...
`"MongoDB Server Information" port:27017` - 100,549 results  
...
`smart install client active` - 10,594 results  
...
`"root@" port:23 -login -password -name -Session` - 1,847 results  
...
`http.title:"Index of /"` - 383,900 results  
...
`"Authentication: disabled" port:445 product:"Samba"` - 275,642 results  


[Filezilla FTP](https://www.shodan.io/search?query=filezilla%20port%3A%2221%22)  
`filezilla port:"21"` - 266,452 results  
...
`http.title:"dashboard"` - 102,677 results  
...
`"Minecraft Server" "protocol 340" port:25565` - 11,715 results  
...
*last updated at: 2023-12-30 10:24:37*
