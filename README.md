...
`"authentication disabled" port:5900,5901` - 6,040 results  
...
`"authentication disabled" "RFB 003.008"` - 5,608 results  
...
`port:20000 source address` - 1,083 results  


[PCWorx](https://www.shodan.io/search?query=port%3A1962%20PLC)  
`port:1962 PLC` - 1,079 results  
...
`[1m[35mWelcome on console` - 73 results  
...
`kibana content-length:217` - 572 results  
...
`"root@" port:23 -login -password -name -Session` - 560 results  
...
`ssl:"Xerox Generic Root"` - 400 results  
...
`http.title:"Index of /"` - 386,508 results  
...
`HACKED-ROUTER-HELP-SOS-HAD-DEFAULT-PASSWORD` - 114 results  
