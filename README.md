...
`"authentication disabled" port:5900,5901` - 5,960 results  
...
`"authentication disabled" "RFB 003.008"` - 5,583 results  
...
`port:2404 asdu address` - 3,634 results  
...
`"DICOM Server Response" port:104` - 1,869 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 17,115 results  


[Cisco Smart Install](https://www.shodan.io/search?query=smart%20install%20client%20active)  
`smart install client active` - 8,002 results  
...
`"Android Debug Bridge" "Device" port:5555` - 4,530 results  
...
`"root@" port:23 -login -password -name -Session` - 600 results  
...
`http.title:"Index of /"` - 376,106 results  
...
`"Authentication: disabled" port:445 product:"Samba"` - 304,139 results  
