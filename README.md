...
`server: "i-Catcher Console"` - 66 results
...
`"Server: EIG Embedded Web Server" "200 Document follows"` - 230,190 results
<br>
...
`port:1911,4911 product:Niagara` - 7,494 results
...
`[1m[35mWelcome on console` - 23 results
<br>
...
`http.title:"- Polycom" "Server: lighttpd"` - 1,746 results
...
`Lantronix password port:30718 -secured` - 531 results
...
`"root@" port:23 -login -password -name -Session` - 332 results
...
`PBX "gateway console" -password port:23` - 136 results
...
`"Docker-Distribution-Api-Version: registry" "200 OK" -gitlab` - 127 results

[Telnet Configuration](https://www.shodan.io/search?query=%22Polycom%20Command%20Shell%22%20-failed%20port%3A23)  
`"Polycom Command Shell" -failed port:23` - 33 results
...
`"Server: EPSON-HTTP" "200 OK"` - 1,445 results
...
`port:23 "Password is not set"` - 306 results
...
`ssl:"Xerox Generic Root"` - 148 results
<br>
...
`port:4444 system32` - 1,574 results
...
`"attention" "encrypted" port:3389` - 36 results
...
`http.title:"dashboard"` - 404,363 results
...
`"Server: Bomgar" "200 OK"` - 279 results
...
*last updated at: 2025-11-15 12:14:49*
