...
`server: "i-Catcher Console"` - 18 results  
...
`"in-tank inventory" port:10001` - 5,953 results  
...
`"authentication disabled" port:5900,5901` - 5,447 results  
...
`[1m[35mWelcome on console` - 57 results  
...
`http.title:"Nordex Control" "Windows 2000 5.0 x86" "Jetty/3.1 (JSP 1.1; Servlet 2.2; java 1.6.0_14)"` - 42 results  


[Electric Vehicle Chargers](https://www.shodan.io/search?query=%22Server%3A%20gSOAP/2.8%22%20%22Content-Length%3A%20583%22)  
`"Server: gSOAP/2.8" "Content-Length: 583"` - 39 results  
...
`"Server: CarelDataServer" "200 Document follows"` - 14 results  
...
`"root@" port:23 -login -password -name -Session` - 1,844 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 1,357 results  
...
`http.title:"Index of /"` - 382,174 results  
...
`HACKED-ROUTER-HELP-SOS-HAD-DEFAULT-PASSWORD` - 131 results  


[Compromised FTP Servers](https://www.shodan.io/search?query=HACKED%20FTP%20server)  
`HACKED FTP server` - 83 results  
...
*last updated at: 2024-01-16 05:20:33*
