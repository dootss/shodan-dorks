...
`product:"Yawcam webcam viewer httpd"` - 518 results  
...
`"in-tank inventory" port:10001` - 5,607 results  
...
`"authentication disabled" port:5900,5901` - 5,028 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 7,252 results  


[Cisco Smart Install](https://www.shodan.io/search?query=smart%20install%20client%20active)  
`smart install client active` - 6,796 results  
...
`"Android Debug Bridge" "Device" port:5555` - 4,037 results  
...
`"dnsmasq-pi-hole" "Recursion: enabled"` - 2,604 results  
