...
`product:"Hikvision IP Camera"` - 2,865,992 results  
...
`title:"IPCam Client"` - 65,219 results  
...
`server: VVTK-HTTP-Server` - 17,034 results  
...
`has_screenshot:true IP Webcam` - 106 results  
...
`port:2404 asdu address` - 3,684 results  
...
`port:20000 source address` - 1,135 results  


[PCWorx](https://www.shodan.io/search?query=port%3A1962%20PLC)  
`port:1962 PLC` - 1,090 results  
...
`[1m[35mWelcome on console` - 82 results  
...
`title:"syncthru web service"` - 1,055 results  
...
`http.title:"Hacked by"` - 558 results  
