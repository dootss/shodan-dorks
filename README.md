...
`WWW-Authenticate: "Merit LILIN Ent. Co., Ltd."` - 1,216 results  
...
`"authentication disabled" port:5900,5901` - 5,873 results  
...
`port:2404 asdu address` - 3,513 results  
...
`port:9600 response code` - 1,964 results  
...
`port:20000 source address` - 1,040 results  


[PCWorx](https://www.shodan.io/search?query=port%3A1962%20PLC)  
`port:1962 PLC` - 1,040 results  
...
`"root@" port:23 -login -password -name -Session` - 605 results  
...
`"Server: EPSON-HTTP" "200 OK"` - 299 results  
...
`http.title:"Index of /"` - 375,632 results  
