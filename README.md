...
`webcam has_screenshot:true` - 244 results  
...
`"authentication disabled" "RFB 003.008"` - 5,311 results  
...
`"DICOM Server Response" port:104` - 1,819 results  
...
`Lantronix password port:30718 -secured` - 646 results  
...
`"root@" port:23 -login -password -name -Session` - 517 results  
...
`"Set-Cookie: mongo-express=" "200 OK"` - 442 results  
...
`PBX "gateway console" -password port:23` - 198 results  
...
`"Server: EPSON-HTTP" "200 OK"` - 333 results  
...
`http.title:"Index of /"` - 378,422 results  
...
`http.title:"Hacked by"` - 528 results  
