...
`webcam has_screenshot:true` - 207 results  
...
`"in-tank inventory" port:10001` - 6,242 results  
...
`"authentication disabled" port:5900,5901` - 5,925 results  
...
`"authentication disabled" "RFB 003.008"` - 5,527 results  
...
`"root@" port:23 -login -password -name -Session` - 590 results  
...
`http.title:"Index of /"` - 373,967 results  
...
`http.title:"Hacked by"` - 551 results  
