...
`title:"IPCam Client"` - 62,603 results  
...
`webcam has_screenshot:true` - 301 results  
...
`"in-tank inventory" port:10001` - 6,084 results  
...
`"authentication disabled" port:5900,5901` - 5,569 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 2,701 results  
...
`"root@" port:23 -login -password -name -Session` - 1,940 results  
...
`Lantronix password port:30718 -secured` - 708 results  
...
`PBX "gateway console" -password port:23` - 169 results  
...
`http.title:"Index of /"` - 385,206 results  
...
`"Authentication: disabled" port:445 product:"Samba"` - 278,841 results  


[Filezilla FTP](https://www.shodan.io/search?query=filezilla%20port%3A%2221%22)  
`filezilla port:"21"` - 270,616 results  
...
*last updated at: 2024-01-24 19:58:02*
