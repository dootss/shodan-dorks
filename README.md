...
`WWW-Authenticate: "Merit LILIN Ent. Co., Ltd."` - 1,335 results  
...
`webcam has_screenshot:true` - 169 results  
...
`"in-tank inventory" port:10001` - 6,077 results  
...
`"Siemens, SIMATIC" port:161` - 2,991 results  
...
`port:9600 response code` - 1,743 results  
...
`[1m[35mWelcome on console` - 94 results  
...
`title:"Slocum Fleet Mission Control"` - 35 results  
...
`port:2455 operating system` - 9 results  
...
`"root@" port:23 -login -password -name -Session` - 1,806 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 1,198 results  


[Accessible Kibana Dashboards](https://www.shodan.io/search?query=kibana%20content-length%3A217)  
`kibana content-length:217` - 713 results  
...
`"HP-ChaiSOE" port:"80"` - 279 results  
...
`http.title:"Index of /"` - 385,589 results  
...
*last updated at: 2023-12-20 13:10:42*
