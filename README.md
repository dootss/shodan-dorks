...
`webcam has_screenshot:true` - 147 results
...
`"Server: EIG Embedded Web Server" "200 Document follows"` - 256,853 results
<br>
...
`"authentication disabled" "RFB 003.008"` - 183,105 results
<br>
...
`"in-tank inventory" port:10001` - 5,371 results
<br>
...
`"DICOM Server Response" port:104` - 2,374 results
...
`"HID VertX" port:4070` - 130 results
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 1,844 results
...
`"root@" port:23 -login -password -name -Session` - 574 results
...
`"Docker-Distribution-Api-Version: registry" "200 OK" -gitlab` - 163 results

[Citrix Virtual Apps](https://www.shodan.io/search?query=%22Citrix%20Applications%3A%22%20port%3A1604)  
`"Citrix Applications:" port:1604` - 147 results
...
`ssl:"Xerox Generic Root"` - 210 results
<br>
...
`Laser Printer FTP Server` - 35 results
...
`http.title:"Index of /"` - 309,718 results
...
`port:4444 system32` - 1,237 results
...
`http.title:"dashboard"` - 401,231 results
...
`"Server: Bomgar" "200 OK"` - 325 results
...
*last updated at: 2025-08-16 12:47:20*
