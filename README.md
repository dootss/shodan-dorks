...
`"Server: EIG Embedded Web Server" "200 Document follows"` - 83,623 results
<br>
...
`"HID VertX" port:4070` - 143 results
...
`"Server: gSOAP/2.8" "Content-Length: 583"` - 8 results
...
`"MongoDB Server Information" port:27017` - 98,251 results

[Default MongoDB Instances](https://www.shodan.io/search?query=mongodb%20port%3A27017)  
`mongodb port:27017` - 98,245 results
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 2,069 results
...
`Lantronix password port:30718 -secured` - 497 results
...
`"root@" port:23 -login -password -name -Session` - 396 results
...
`PBX "gateway console" -password port:23` - 181 results
...
`ssl:"Xerox Generic Root"` - 237 results
<br>
...
`http.title:"dashboard"` - 342,021 results
...
*last updated at: 2025-06-07 15:16:39*
