...
`"authentication disabled" port:5900,5901` - 5,791 results  
...
`"dnsmasq-pi-hole" "Recursion: enabled"` - 2,637 results  
...
`kibana content-length:217` - 549 results  
...
`"root@" port:23 -login -password -name -Session` - 488 results  
...
`"Citrix Applications:" port:1604` - 304 results  
...
`"QuickBooks files OverNetwork" -unix port:445` - 39 results  
...
`http.title:"dashboard"` - 107,673 results  
...
`"Minecraft Server" "protocol 340" port:25565` - 10,660 results  
