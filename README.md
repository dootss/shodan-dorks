...
`Netwave IP Camera Content-Length: 2574` - 1,304 results  
...
`product:"Yawcam webcam viewer httpd"` - 637 results  
...
`"authentication disabled" "RFB 003.008"` - 5,497 results  
...
`"Siemens, SIMATIC" port:161` - 3,176 results  


[Omron FINS](https://www.shodan.io/search?query=port%3A9600%20response%20code)  
`port:9600 response code` - 1,974 results  
...
`port:"9200" all:elastic` - 32,156 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 19,249 results  
...
`http.title:"- Polycom" "Server: lighttpd"` - 3,652 results  
...
`"root@" port:23 -login -password -name -Session` - 567 results  
...
`http 200 server epson -upnp` - 3,085 results  
...
`http.title:"Index of /"` - 377,640 results  
