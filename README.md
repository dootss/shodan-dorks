...
`has_screenshot:true IP Webcam` - 73 results  
...
`"authentication disabled" port:5900,5901` - 6,161 results  
...
`"in-tank inventory" port:10001` - 5,933 results  
...
`[1m[35mWelcome on console` - 81 results  
...
`http.title:"Nordex Control" "Windows 2000 5.0 x86" "Jetty/3.1 (JSP 1.1; Servlet 2.2; java 1.6.0_14)"` - 35 results  
...
`Lantronix password port:30718 -secured` - 650 results  
...
`"root@" port:23 -login -password -name -Session` - 568 results  
...
`"Polycom Command Shell" -failed port:23` - 46 results  
...
`"QuickBooks files OverNetwork" -unix port:445` - 37 results  
