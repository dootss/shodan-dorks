...
`server: "i-Catcher Console"` - 21 results  
...
`"in-tank inventory" port:10001` - 6,079 results  
...
`"authentication disabled" port:5900,5901` - 5,557 results  
...
`port:5006,5007 product:mitsubishi` - 184 results  
...
`"Server: gSOAP/2.8" "Content-Length: 583"` - 47 results  
...
`http.title:"Nordex Control" "Windows 2000 5.0 x86" "Jetty/3.1 (JSP 1.1; Servlet 2.2; java 1.6.0_14)"` - 40 results  
...
`mikrotik streetlight` - 17 results  
...
`"Server: CarelDataServer" "200 Document follows"` - 17 results  
...
`"root@" port:23 -login -password -name -Session` - 1,955 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 1,891 results  
...
`port:23 "Password is not set"` - 441 results  
...
`http.title:"Index of /"` - 385,480 results  
...
`HACKED-ROUTER-HELP-SOS-HAD-DEFAULT-PASSWORD` - 130 results  


[Compromised FTP Servers](https://www.shodan.io/search?query=HACKED%20FTP%20server)  
`HACKED FTP server` - 86 results  
...
*last updated at: 2024-01-23 00:04:48*
