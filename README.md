...
`"authentication disabled" port:5900,5901` - 5,813 results  
...
`"authentication disabled" "RFB 003.008"` - 5,408 results  
...
`"Siemens, SIMATIC" port:161` - 3,137 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 18,683 results  
...
`http.title:"- Polycom" "Server: lighttpd"` - 3,634 results  
...
`kibana content-length:217` - 559 results  
...
`"root@" port:23 -login -password -name -Session` - 549 results  
...
`http.title:"Index of /"` - 381,225 results  
...
`"Authentication: disabled" port:445 product:"Samba"` - 282,010 results  


[Filezilla FTP](https://www.shodan.io/search?query=filezilla%20port%3A%2221%22)  
`filezilla port:"21"` - 257,920 results  
...
`http.title:"control panel"` - 66,414 results  
