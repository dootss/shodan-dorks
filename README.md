...
`"in-tank inventory" port:10001` - 6,223 results  
...
`"authentication disabled" port:5900,5901` - 5,783 results  
...
`"authentication disabled" "RFB 003.008"` - 5,392 results  
...
`port:20000 source address` - 1,004 results  
...
`product:MySQL` - 3,340,981 results  
...
`"root@" port:23 -login -password -name -Session` - 528 results  
...
`ssl:"Xerox Generic Root"` - 365 results  
...
`http.title:"Index of /"` - 378,291 results  
...
`"Authentication: disabled" port:445 product:"Samba"` - 263,117 results  


[Filezilla FTP](https://www.shodan.io/search?query=filezilla%20port%3A%2221%22)  
`filezilla port:"21"` - 256,422 results  
