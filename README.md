...
`Netwave IP Camera Content-Length: 2574` - 1,289 results  
...
`"authentication disabled" "RFB 003.008"` - 5,356 results  
...
`"DICOM Server Response" port:104` - 1,845 results  


[DNP3](https://www.shodan.io/search?query=port%3A20000%20source%20address)  
`port:20000 source address` - 1,104 results  
...
`port:"9200" all:elastic` - 31,858 results  
...
`"root@" port:23 -login -password -name -Session` - 553 results  
...
`PBX "gateway console" -password port:23` - 212 results  
...
`http.title:"Index of /"` - 374,495 results  
