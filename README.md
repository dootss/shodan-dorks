...
`"authentication disabled" "RFB 003.008"` - 5,820 results  
...
`"authentication disabled" port:5900,5901` - 5,736 results  
...
`port:2404 asdu address` - 3,530 results  
...
`port:9600 response code` - 1,859 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 16,064 results  


[Cisco Smart Install](https://www.shodan.io/search?query=smart%20install%20client%20active)  
`smart install client active` - 7,272 results  
...
`"Android Debug Bridge" "Device" port:5555` - 4,286 results  
