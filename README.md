...
`WWW-Authenticate: "Merit LILIN Ent. Co., Ltd"` - 529 results
...
`"Server: EIG Embedded Web Server" "200 Document follows"` - 155,368 results
<br>
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 1,359 results

[Android Root Bridges](https://www.shodan.io/search?query=%22Android%20Debug%20Bridge%22%20%22Device%22%20port%3A5555)  
`"Android Debug Bridge" "Device" port:5555` - 1,130 results
...
`"root@" port:23 -login -password -name -Session` - 283 results
...
`"Docker-Distribution-Api-Version: registry" "200 OK" -gitlab` - 126 results
...
`"Server: EPSON-HTTP" "200 OK"` - 985 results
...
`ssl:"Xerox Generic Root"` - 163 results
<br>
...
`Printer Type: Lexmark` - 120 results
...
`port:4444 system32` - 1,221 results
...
`http.title:"Hacked by"` - 207 results
...
`http.title:"dashboard"` - 313,967 results
...
`"Server: Bomgar" "200 OK"` - 260 results
...
*last updated at: 2026-01-11 23:29:29*
