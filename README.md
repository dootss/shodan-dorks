...
`200 ok dvr port:"81"` - 10,072 results  
...
`"authentication disabled" port:5900,5901` - 6,115 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 13,580 results  


[Cisco Smart Install](https://www.shodan.io/search?query=smart%20install%20client%20active)  
`smart install client active` - 8,186 results  
...
`"Set-Cookie: mongo-express=" "200 OK"` - 447 results  
...
`Server: CANON HTTP Server` - 10,201 results  
...
`title:"syncthru web service"` - 1,020 results  
...
`filezilla port:"21"` - 258,154 results  
...
`http.title:"dashboard"` - 365,237 results  
