...
`WWW-Authenticate: "Merit LILIN Ent. Co., Ltd"` - 597 results
...
`has_screenshot:true IP Webcam` - 10 results
<br>
...
`"Server: EIG Embedded Web Server" "200 Document follows"` - 157,328 results
<br>
...
`"authentication disabled" port:5900,5901` - 877 results
<br>
...
`http.title:"Nordex Control" "Windows 2000 5.0 x86" "Jetty/3.1 (JSP 1.1; Servlet 2.2; java 1.6.0_14)"` - 13 results
...
`"root@" port:23 -login -password -name -Session` - 229 results
...
`Printer Type: Lexmark` - 51 results
...
`"Location: /main/main.html" debut` - 18 results
...
`port:4444 system32` - 1,327 results
...
`HACKED-ROUTER-HELP-SOS-HAD-DEFAULT-PASSWORD` - 13 results

[Compromised FTP Servers](https://www.shodan.io/search?query=HACKED%20FTP%20server)  
`HACKED FTP server` - 10 results
...
`http.title:"dashboard"` - 335,374 results
...
*last updated at: 2026-08-05 16:24:20*
