...
`"Server: EIG Embedded Web Server" "200 Document follows"` - 255,225 results
<br>
...
`"authentication disabled" "RFB 003.008"` - 209,351 results
<br>
...
`port:9600 response code` - 1,772 results
...
`http.title:"Nordex Control" "Windows 2000 5.0 x86" "Jetty/3.1 (JSP 1.1; Servlet 2.2; java 1.6.0_14)"` - 28 results
...
`Lantronix password port:30718 -secured` - 455 results
...
`"root@" port:23 -login -password -name -Session` - 392 results
...
`"Set-Cookie: mongo-express=" "200 OK"` - 331 results
...
`"Docker-Distribution-Api-Version: registry" "200 OK" -gitlab` - 181 results

[Citrix Virtual Apps](https://www.shodan.io/search?query=%22Citrix%20Applications%3A%22%20port%3A1604)  
`"Citrix Applications:" port:1604` - 160 results
...
`"Server: EPSON-HTTP" "200 OK"` - 1,483 results
...
`Printer Type: Lexmark` - 125 results
...
`http.title:"Index of /"` - 296,972 results
...
`http.title:"Hacked by"` - 442 results
...
`HACKED-ROUTER-HELP-SOS-HAD-DEFAULT-PASSWORD` - 74 results
...
`http.title:"dashboard"` - 390,154 results
...
*last updated at: 2025-07-05 11:23:31*
