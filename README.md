...
`"authentication disabled" port:5900,5901` - 5,804 results  
...
`"Siemens, SIMATIC" port:161` - 3,112 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 18,004 results  
...
`http.title:"- Polycom" "Server: lighttpd"` - 3,631 results  
...
`PBX "gateway console" -password port:23` - 204 results  
...
`http 200 server epson -upnp` - 2,996 results  
...
`"attention" "encrypted" port:3389` - 13 results  
...
`http.title:"control panel"` - 66,516 results  
