...
`Netwave IP Camera Content-Length: 2574` - 1,263 results  
...
`"authentication disabled" port:5900,5901` - 5,918 results  
...
`"authentication disabled" "RFB 003.008"` - 5,489 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 18,646 results  
...
`http 200 server epson -upnp` - 3,031 results  
...
`ssl:"Xerox Generic Root"` - 424 results  
...
`http.title:"Index of /"` - 372,481 results  
...
`"Authentication: disabled" port:445 product:"Samba"` - 294,797 results  


[Filezilla FTP](https://www.shodan.io/search?query=filezilla%20port%3A%2221%22)  
`filezilla port:"21"` - 255,779 results  
...
`http.title:"control panel"` - 66,830 results  
