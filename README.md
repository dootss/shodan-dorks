...
`WWW-Authenticate: "Merit LILIN Ent. Co., Ltd."` - 1,219 results  
...
`webcam has_screenshot:true` - 185 results  
...
`"authentication disabled" port:5900,5901` - 5,842 results  
...
`"authentication disabled" "RFB 003.008"` - 5,488 results  
...
`port:20000 source address` - 1,102 results  
...
`port:5006,5007 product:mitsubishi` - 248 results  
...
`"Server: gSOAP/2.8" "Content-Length: 583"` - 31 results  
...
`"HP-ChaiSOE" port:"80"` - 137 results  
...
`http.title:"Index of /"` - 373,561 results  
...
`http.title:"Hacked by"` - 552 results  
...
`HACKED-ROUTER-HELP-SOS-HAD-DEFAULT-PASSWORD` - 99 results  
