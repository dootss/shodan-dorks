...
`server: VVTK-HTTP-Server` - 15,962 results  
...
`"authentication disabled" "RFB 003.008"` - 6,480 results  
...
`"authentication disabled" port:5900,5901` - 5,485 results  
...
`port:2404 asdu address` - 3,356 results  
...
`port:"9200" all:elastic` - 30,879 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 12,378 results  


[Cisco Smart Install](https://www.shodan.io/search?query=smart%20install%20client%20active)  
`smart install client active` - 7,987 results  
...
`http.title:"- Polycom" "Server: lighttpd"` - 3,865 results  
...
`Lantronix password port:30718 -secured` - 668 results  
...
`"Polycom Command Shell" -failed port:23` - 65 results  
...
`"Authentication: disabled" port:445 product:"Samba"` - 250,327 results  
...
*last updated at: 2024-02-21 11:34:34*
