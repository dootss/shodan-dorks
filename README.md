...
`product:"Yawcam webcam viewer httpd"` - 607 results  
...
`has_screenshot:true IP Webcam` - 89 results  
...
`"in-tank inventory" port:10001` - 6,319 results  
...
`"authentication disabled" port:5900,5901` - 6,163 results  
...
`"authentication disabled" "RFB 003.008"` - 5,796 results  
...
`http.title:"Nordex Control" "Windows 2000 5.0 x86" "Jetty/3.1 (JSP 1.1; Servlet 2.2; java 1.6.0_14)"` - 43 results  
...
`Lantronix password port:30718 -secured` - 657 results  
...
`"Set-Cookie: mongo-express=" "200 OK"` - 450 results  
...
`PBX "gateway console" -password port:23` - 219 results  
...
`"Docker-Distribution-Api-Version: registry" "200 OK" -gitlab` - 155 results  


[Telnet Configuration](https://www.shodan.io/search?query=%22Polycom%20Command%20Shell%22%20-failed%20port%3A23)  
`"Polycom Command Shell" -failed port:23` - 36 results  
...
`"Server: EPSON-HTTP" "200 OK"` - 311 results  
...
`http.title:"Index of /"` - 387,722 results  
...
`HACKED-ROUTER-HELP-SOS-HAD-DEFAULT-PASSWORD` - 112 results  
