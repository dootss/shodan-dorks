...
`product:"Hikvision IP Camera"` - 2,847,662 results  
...
`"in-tank inventory" port:10001` - 5,981 results  
...
`"authentication disabled" port:5900,5901` - 5,488 results  
...
`port:2404 asdu address` - 3,283 results  
...
`port:9600 response code` - 1,701 results  


[PCWorx](https://www.shodan.io/search?query=port%3A1962%20PLC)  
`port:1962 PLC` - 906 results  
...
`[1m[35mWelcome on console` - 60 results  
...
`kibana content-length:217` - 662 results  
...
`"Server: EPSON-HTTP" "200 OK"` - 368 results  
...
`"Location: /main/main.html" debut` - 60 results  
...
`http.title:"Index of /"` - 384,077 results  
...
`"QuickBooks files OverNetwork" -unix port:445` - 47 results  
...
*last updated at: 2024-01-17 04:16:46*
