...
`"in-tank inventory" port:10001` - 6,012 results  
...
`port:9600 response code` - 1,707 results  


[PCWorx](https://www.shodan.io/search?query=port%3A1962%20PLC)  
`port:1962 PLC` - 892 results  
...
`"Server: gSOAP/2.8" "Content-Length: 583"` - 44 results  
...
`"root@" port:23 -login -password -name -Session` - 1,900 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 1,362 results  
...
`PBX "gateway console" -password port:23` - 166 results  
...
`"Docker-Distribution-Api-Version: registry" "200 OK" -gitlab` - 147 results  
...
`title:"Weave Scope" http.favicon.hash:567176827` - 48 results  
...
`http.title:"Index of /"` - 381,561 results  
...
`HACKED-ROUTER-HELP-SOS-HAD-DEFAULT-PASSWORD` - 126 results  
...
*last updated at: 2024-01-21 11:34:12*
