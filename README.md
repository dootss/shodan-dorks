...
`"in-tank inventory" port:10001` - 5,987 results  
...
`"authentication disabled" port:5900,5901` - 5,492 results  
...
`port:9600 response code` - 1,705 results  


[PCWorx](https://www.shodan.io/search?query=port%3A1962%20PLC)  
`port:1962 PLC` - 895 results  
...
`"root@" port:23 -login -password -name -Session` - 1,890 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 1,379 results  
...
`kibana content-length:217` - 654 results  
...
`http.title:"Index of /"` - 383,242 results  
...
*last updated at: 2024-01-19 14:44:07*
