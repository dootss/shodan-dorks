...
`WWW-Authenticate: "Merit LILIN Ent. Co., Ltd."` - 1,160 results  
...
`"authentication disabled" port:5900,5901` - 5,926 results  
...
`"in-tank inventory" port:10001` - 5,896 results  
...
`"Siemens, SIMATIC" port:161` - 3,040 results  
...
`port:20000 source address` - 993 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 7,271 results  


[Cisco Smart Install](https://www.shodan.io/search?query=smart%20install%20client%20active)  
`smart install client active` - 6,882 results  
...
`"Set-Cookie: mongo-express=" "200 OK"` - 397 results  
...
`Server: CANON HTTP Server` - 9,442 results  
...
`title:"syncthru web service"` - 965 results  
...
`hacked-router-help-sos` - 726 results  
