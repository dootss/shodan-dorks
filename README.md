...
`webcam has_screenshot:true` - 306 results  
...
`"in-tank inventory" port:10001` - 6,066 results  
...
`"authentication disabled" port:5900,5901` - 5,694 results  
...
`port:2404 asdu address` - 3,258 results  
...
`[1m[35mWelcome on console` - 54 results  
...
`"root@" port:23 -login -password -name -Session` - 1,907 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 1,291 results  
...
`Lantronix password port:30718 -secured` - 700 results  
...
`"Set-Cookie: mongo-express=" "200 OK"` - 497 results  
...
`http.title:"Index of /"` - 382,498 results  
...
`http.title:"Hacked by"` - 718 results  
...
`HACKED-ROUTER-HELP-SOS-HAD-DEFAULT-PASSWORD` - 150 results  
...
*last updated at: 2024-01-06 04:10:25*
