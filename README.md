...
`product:"Yawcam webcam viewer httpd"` - 556 results  
...
`"authentication disabled" port:5900,5901` - 6,122 results  
...
`"in-tank inventory" port:10001` - 5,902 results  
...
`port:20000 source address` - 1,000 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 7,200 results  
...
`"Citrix Applications:" port:1604` - 310 results  
...
`http 200 server epson -upnp` - 1,911 results  
