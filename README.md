...
`webcam has_screenshot:true` - 175 results  
...
`"authentication disabled" port:5900,5901` - 5,847 results  
...
`"root@" port:23 -login -password -name -Session` - 1,877 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 1,285 results  
...
`ssl:"Xerox Generic Root"` - 412 results  
...
`"HP-ChaiSOE" port:"80"` - 224 results  
...
`http.title:"Index of /"` - 385,900 results  
...
`HACKED-ROUTER-HELP-SOS-HAD-DEFAULT-PASSWORD` - 142 results  


[Compromised FTP Servers](https://www.shodan.io/search?query=HACKED%20FTP%20server)  
`HACKED FTP server` - 88 results  
...
`http.html:"* The wp-config.php creation script uses this file"` - 16 results  
...
*last updated at: 2023-12-30 01:00:34*
