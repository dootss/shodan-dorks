...
`server: GeoHttpServer` - 40,957 results  
...
`"authentication disabled" "RFB 003.008"` - 5,393 results  
...
`port:"9200" all:elastic` - 31,365 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 17,433 results  
...
`http.title:"- Polycom" "Server: lighttpd"` - 3,647 results  
...
`"root@" port:23 -login -password -name -Session` - 533 results  
...
`PBX "gateway console" -password port:23` - 195 results  
...
`http 200 server epson -upnp` - 2,936 results  
...
`ssl:"Xerox Generic Root"` - 350 results  
...
`http.title:"Index of /"` - 377,451 results  
...
`filezilla port:"21"` - 256,507 results  
...
`http.title:"control panel"` - 65,926 results  
