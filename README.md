...
`WWW-Authenticate: "Merit LILIN Ent. Co., Ltd."` - 1,349 results  
...
`"in-tank inventory" port:10001` - 6,099 results  
...
`port:2404 asdu address` - 3,037 results  
...
`[1m[35mWelcome on console` - 96 results  
...
`title:"Slocum Fleet Mission Control"` - 44 results  
...
`port:2455 operating system` - 12 results  
...
`"root@" port:23 -login -password -name -Session` - 1,840 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 1,203 results  
...
`"Set-Cookie: mongo-express=" "200 OK"` - 542 results  
...
`ssl:"Xerox Generic Root"` - 439 results  
...
`"HP-ChaiSOE" port:"80"` - 287 results  
...
`http.title:"Index of /"` - 387,459 results  
...
`HACKED-ROUTER-HELP-SOS-HAD-DEFAULT-PASSWORD` - 137 results  


[Compromised FTP Servers](https://www.shodan.io/search?query=HACKED%20FTP%20server)  
`HACKED FTP server` - 95 results  
...
*last updated at: 2023-12-20 02:45:48*
