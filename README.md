...
`Netwave IP Camera Content-Length: 2574` - 1,274 results  
...
`"authentication disabled" "RFB 003.008"` - 5,303 results  
...
`"root@" port:23 -login -password -name -Session` - 544 results  
...
`"Server: EPSON-HTTP" "200 OK"` - 321 results  
...
`http.title:"Index of /"` - 373,995 results  
...
`"Authentication: disabled" port:445 product:"Samba"` - 278,712 results  


[Filezilla FTP](https://www.shodan.io/search?query=filezilla%20port%3A%2221%22)  
`filezilla port:"21"` - 255,044 results  
