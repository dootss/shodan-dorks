...
`webcam has_screenshot:true` - 126 results
...
`has_screenshot:true IP Webcam` - 58 results
<br>
...
`"Server: EIG Embedded Web Server" "200 Document follows"` - 244,827 results
<br>
...
`"authentication disabled" "RFB 003.008"` - 174,734 results
<br>
...
`"DICOM Server Response" port:104` - 2,325 results

[Omron FINS](https://www.shodan.io/search?query=port%3A9600%20response%20code)  
`port:9600 response code` - 1,700 results
...
`http.title:"Nordex Control" "Windows 2000 5.0 x86" "Jetty/3.1 (JSP 1.1; Servlet 2.2; java 1.6.0_14)"` - 27 results

[C4 Max Commercial Vehicle GPS Trackers](https://www.shodan.io/search?query=%5B1m%5B35mWelcome%20on%20console)  
`[1m[35mWelcome on console` - 26 results
<br>
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 1,856 results
...
`"root@" port:23 -login -password -name -Session` - 577 results
...
`"Docker-Distribution-Api-Version: registry" "200 OK" -gitlab` - 173 results
...
`ssl:"Xerox Generic Root"` - 191 results
<br>
...
`Printer Type: Lexmark` - 134 results
...
`"Location: /main/main.html" debut` - 47 results
...
`http.title:"Index of /"` - 308,538 results
...
`bitcoin has_screenshot:true` - 2,916 results
<br>
...
`http.title:"dashboard"` - 389,149 results
...
*last updated at: 2025-08-30 15:22:34*
