...
`has_screenshot:true IP Webcam` - 17 results
<br>
...
`title:"blue iris remote view"` - 8 results
...
`"Server: EIG Embedded Web Server" "200 Document follows"` - 191,673 results
<br>
...
`"authentication disabled" port:5900,5901` - 1,844 results
<br>
...
`[1m[35mWelcome on console` - 13 results
<br>
...
`http.title:"Nordex Control" "Windows 2000 5.0 x86" "Jetty/3.1 (JSP 1.1; Servlet 2.2; java 1.6.0_14)"` - 10 results

[Open ATM](https://www.shodan.io/search?query=NCR%20Port%3A%22161%22)  
`NCR Port:"161"` - 7 results
...
`mikrotik streetlight` - 2 results
...
`Lantronix password port:30718 -secured` - 496 results
...
`"Docker-Distribution-Api-Version: registry" "200 OK" -gitlab` - 206 results
...
`title:"Weave Scope" http.favicon.hash:567176827` - 2 results
...
`title:"OctoPrint" -title:"Login" http.favicon.hash:1307375944` - 7 results

[Printers with FTP Access](https://www.shodan.io/search?query=Laser%20Printer%20FTP%20Server)  
`Laser Printer FTP Server` - 2 results
...
`"QuickBooks files OverNetwork" -unix port:445` - 26 results
...
`port:4444 system32` - 1,143 results
...
`HACKED-ROUTER-HELP-SOS-HAD-DEFAULT-PASSWORD` - 15 results
...
`http.title:"dashboard"` - 405,854 results
...
`net:175.45.176.0/22,210.52.109.0/24,77.94.35.0/24` - 34 results
...
*last updated at: 2026-09-15 21:04:07*
