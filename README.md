...
`"authentication disabled" port:5900,5901` - 5,452 results  
...
`port:"9200" all:elastic` - 30,869 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 9,305 results  


[Cisco Smart Install](https://www.shodan.io/search?query=smart%20install%20client%20active)  
`smart install client active` - 7,562 results  
...
`http.title:"- Polycom" "Server: lighttpd"` - 3,839 results  
...
`"dnsmasq-pi-hole" "Recursion: enabled"` - 2,577 results  
...
`"root@" port:23 -login -password -name -Session` - 1,822 results  
...
`"Set-Cookie: mongo-express=" "200 OK"` - 479 results  
...
*last updated at: 2024-03-03 19:14:25*
