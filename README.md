...
`"in-tank inventory" port:10001` - 5,534 results  
...
`"authentication disabled" port:5900,5901` - 4,805 results  
...
`"Siemens, SIMATIC" port:161` - 2,729 results  
...
`port:9600 response code` - 1,646 results  
...
`port:"9200" all:elastic` - 28,460 results  
...
`"X-Jenkins" "Set-Cookie: JSESSIONID" http.title:"Dashboard"` - 6,719 results  
...
`http 200 server epson -upnp` - 1,824 results  
...
`"Authentication: disabled" port:445 product:"Samba"` - 154,468 results  


[Open Lists on Port 80](https://www.shodan.io/search?query=port%3A80%20title%3A%22Index%20of%20/%22)  
`port:80 title:"Index of /"` - 138,331 results  
...
`"Anonymous access allowed" port:"21"` - 30,849 results  
...
*last updated at: 2024-03-30 19:48:36*
