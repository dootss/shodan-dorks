...
`"in-tank inventory" port:10001` - 6,109 results  
...
`"authentication disabled" port:5900,5901` - 5,588 results  
...
`port:2404 asdu address` - 3,342 results  
...
`port:9600 response code` - 1,741 results  


[PCWorx](https://www.shodan.io/search?query=port%3A1962%20PLC)  
`port:1962 PLC` - 872 results  
...
`"root@" port:23 -login -password -name -Session` - 1,952 results  
...
`kibana content-length:217` - 648 results  
...
`"Polycom Command Shell" -failed port:23` - 71 results  
...
`http.title:"Index of /"` - 390,703 results  
...
*last updated at: 2024-01-30 16:53:10*
